...
local function create(zone, options)
  local pie = { zone=zone, options=options, bitmap = Bitmap.open("/IMAGES/cat45.png") }
  return pie
end
...
function refresh(pie)
  ...
  -- Speed
  lcd.drawChannel(pie.zone.x+pie.zone.w, pie.zone.y+pie.zone.h-48, "RPM2", SMLSIZE+TEXT_COLOR+RIGHT)
  lcd.drawGauge(pie.zone.x+pie.zone.w-118, pie.zone.y+pie.zone.h-46, 40, 10, getValue('RPM2'), 14000, CURVE_COLOR)
  ...
  lcd.drawGauge(pie.zone.x+pie.zone.w-118, pie.zone.y+pie.zone.h-10, 40, 10, getValue('RPM1'), 14000, CURVE_COLOR)
  lcd.drawChannel(pie.zone.x+pie.zone.w, pie.zone.y+pie.zone.h-12, "RPM1", SMLSIZE+TEXT_COLOR+RIGHT)
...
return { name="cat45", options=options, create=create, update=update, refresh=refresh, background=background }
