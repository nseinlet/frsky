...
local function runWarningPage(event)
  ...
  lcd.drawText(0, 10, "Attention: ceci demarrera la calibration du SxR", SMLSIZE)
  lcd.drawText(0, 18, "Ceci doit etre fait une seule fois. Votre SxR", SMLSIZE)
  lcd.drawText(0, 26, "doit etre alimente et pose sur une surface", SMLSIZE)
  lcd.drawText(0, 34, "plane et de niveau (bureau, plan de travail, ...).", SMLSIZE)
  lcd.drawText(0, 46, "Appuyer sur [ENT] quand vous etes pret", SMLSIZE)
  lcd.drawText(0, 56, "Appuyer sur [EXIT] pour annuler", SMLSIZE)
...
local function runCalibrationPage(event)
  ...
  if(calibrationStep < 6) then
    lcd.drawText(0, 9, "Orienter le SxR comme sur l'image", 0)
    ...
    lcd.drawText(0, 56, "Appuyer [ENT] quand pret", attr)
  else
    lcd.drawText(0, 9, "Calibration terminee", 0)
    lcd.drawPixmap(10, 19, "bmp/done.bmp")
    lcd.drawText(0, 56, "Appuyer [EXIT] quand pret", attr)
  end
