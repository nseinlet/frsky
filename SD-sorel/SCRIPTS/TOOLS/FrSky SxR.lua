...
local configFields = {
    { "Type d'Aile:", COMBO, 0x80, nil, { "Normal", "Delta", "VTail" } },
    { "Type Montage:", COMBO, 0x81, nil, { "Horz", "Horz rev.", "Vert", "Vert rev." } },
}
...
local settingsFields = {
    {"Fonctions SxR:", COMBO, 0x9C, nil, { "Desactive", "Active" } },
    {"Quick Mode:", COMBO, 0xAA, nil, { "Desactive", "Active" } },
    {"Mode VOIE5:", COMBO, 0xA8, nil, { "AIL2", "AUX1" } },
    {"Mode VOIE6:", COMBO, 0xA9, nil, { "PRF2", "AUX2" } },
    {"Direction AIL:", COMBO, 0x82, nil, { "Normal", "Invers" }, { 255, 0 } },
    {"Direction PRF:", COMBO, 0x83, nil, { "Normal", "Invers" }, { 255, 0 } },
    {"Direction DIR:", COMBO, 0x84, nil, { "Normal", "Invers" }, { 255, 0 } },
    {"Direction AIL2:", COMBO, 0x9A, nil, { "Normal", "Invers" }, { 255, 0 } },
    {"Direction PRF2:", COMBO, 0x9B, nil, { "Normal", "Invers" }, { 255, 0 } },
    {"Gain stabilisation AIL:", VALUE, 0x85, nil, 0, 200, "%"},
    {"Gain stabilisation PRF:", VALUE, 0x86, nil, 0, 200, "%"},
    {"Gain stabilisation DIR:", VALUE, 0x87, nil, 0, 200, "%"},
    {"Gain remise a plat AIL:", VALUE, 0x88, nil, 0, 200, "%"},
    {"Gain remise a plat PRF:", VALUE, 0x89, nil, 0, 200, "%"},
    {"Gain torque roll PRF:", VALUE, 0x8C, nil, 0, 200, "%"},
    {"Gain torque roll DIR:", VALUE, 0x8D, nil, 0, 200, "%"},
    {"Gain vol tranche AIL:", VALUE, 0x8E, nil, 0, 200, "%"},
    {"Gain vol tranche DIR:", VALUE, 0x90, nil, 0, 200, "%"},
    {"Offset remise a plat AIL:", VALUE, 0x91, nil, -20, 20, "%", 0x6C},
    {"Offset remise a plat PRF:", VALUE, 0x92, nil, -20, 20, "%", 0x6C},
    {"Offset torque roll PRF:", VALUE, 0x95, nil, -20, 20, "%", 0x6C},
    {"Offset torque roll DIR:", VALUE, 0x96, nil, -20, 20, "%", 0x6C},
    {"Offset vol tranche AIL:", VALUE, 0x97, nil, -20, 20, "%", 0x6C},
    {"Offset vol tranche DIR:", VALUE, 0x99, nil, -20, 20, "%", 0x6C},
}
...
local function runConfigPage(event)
    ...
    if LCD_W == 128 then
        local mountText = { "Etiquette vers le haut", "Etiquette vers le bas", "Etiquette a gauche quand", "Etiquette a droite quand" }
        if fields[2][4] ~= nil then
            lcd.drawText(1, 30, "Pins vers la queue")
            lcd.drawText(1, 40, mountText[1 + fields[2][4]])
            if fields[2][4] > 1 then
                lcd.drawText(1, 50, "on regarde depuis la queue")
            end
        end
    else
